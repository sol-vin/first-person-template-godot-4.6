...
@export_tool_button("Randomize", "Callable") var randomize_action = randomize_face_data
@export_tool_button("Clear", "Callable") var clear_action = clear_face_data
...
const DEFAULT_PREBUILT_MATERIALS = [
	...
	"res://textures/full/full_blue.tres",
	"res://textures/full/full_brown.tres",
	"res://textures/full/full_dark_blue.tres",
	"res://textures/full/full_dark_brown.tres",
	"res://textures/full/full_dark_green.tres",
	"res://textures/full/full_dark_grey.tres",
	"res://textures/full/full_dark_magenta.tres",
	"res://textures/full/full_dark_pink.tres",
	"res://textures/full/full_dark_purple.tres",
	"res://textures/full/full_green.tres",
	"res://textures/full/full_grey.tres",
	"res://textures/full/full_indigo.tres",
	"res://textures/full/full_light_blue.tres",
	"res://textures/full/full_light_grey.tres",
	"res://textures/full/full_lime_green.tres",
	"res://textures/full/full_magenta.tres",
	"res://textures/full/full_med_green.tres",
	"res://textures/full/full_midnight.tres",
	"res://textures/full/full_orange.tres",
	"res://textures/full/full_peach.tres",
	"res://textures/full/full_pink.tres",
	"res://textures/full/full_purple.tres",
	"res://textures/full/full_red.tres",
	"res://textures/full/full_sky_blue.tres",
	"res://textures/full/full_tan.tres",
	"res://textures/full/full_white.tres",
	"res://textures/full/full_yellow.tres",
	"res://textures/full/trans_light_blue.tres",
...
@export var ignored_materials = DEFAULT_IGNORED_MATERIALS

## Materials that will be included, but not surface collected during a rebuild
@export var prebuilt_materials = DEFAULT_PREBUILT_MATERIALS
...
static func make_face(id : int, vertex_ids : Array[int]):
	return {"id" : id, "vertex_ids" : vertex_ids}
	
static func make_polygon(face_ids : Array[int], vertex_ids : Array[int]):
	return {"face_ids" : face_ids, "vertex_ids" : vertex_ids}

static func color(mesh : ArrayMesh, normal_diff = 0.001, vertex_diff = 0.001, ignored_materials = DEFAULT_IGNORED_MATERIALS, prebuilt_materials = DEFAULT_PREBUILT_MATERIALS) -> ArrayMesh:
	...
	for surface_id in mesh.get_surface_count():
		# Should we ignore the whole surface
		var ignore_surface = false
		for material in ignored_materials:
			if mesh.surface_get_material(surface_id).resource_path == material:
				ignore_surface = true
				break
		if ignore_surface:
			continue
		
		# Is it a prebuilt material
		var prebuilt_surface = false
		for material in prebuilt_materials:
			if mesh.surface_get_material(surface_id).resource_path == material:
				polygon_surfaces.push_back([])
				final_surface_id += 1
				prebuilt_surface = true
				break
	...
	for surface_id in mesh.get_surface_count():
		# Should this surface be ignored?
		var ignore_surface = false
		for material in ignored_materials:
			if mesh.surface_get_material(surface_id).resource_path == material:
				ignore_surface = true
		...
		# If its prebuilt just copy the data
		var prebuilt_surface = false
		for material in prebuilt_materials:
			if mesh.surface_get_material(surface_id).resource_path == material:
				var mesh_data : MeshDataTool = mesh_data_surfaces[new_mesh.get_surface_count()]
				var surface_tool = SurfaceTool.new()
				surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)
				for face_id in mesh_data.get_face_count():
					var v1 = mesh_data.get_vertex(mesh_data.get_face_vertex(face_id, 0))
					var v2 = mesh_data.get_vertex(mesh_data.get_face_vertex(face_id, 1))
					var v3 = mesh_data.get_vertex(mesh_data.get_face_vertex(face_id, 2))
					var v1_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face_id, 0))
					var v2_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face_id, 1))
					var v3_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face_id, 2))
				...
				new_mesh.surface_set_material(new_mesh.get_surface_count() - 1, mesh.surface_get_material(surface_id))
				prebuilt_surface = true
				break
		if prebuilt_surface:
			continue

		var mesh_data = mesh_data_surfaces[new_mesh.get_surface_count()]
		var surface_tool = SurfaceTool.new()
		surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)
		
		for polygon_id in polygon_surfaces[new_mesh.get_surface_count()].size():
			var color = Color(randf(), randf(), randf(), 1.0)
			surface_tool.set_color(color)
			for face in polygon_surfaces[new_mesh.get_surface_count()][polygon_id]["face_ids"]:
...
func rebuild_face_data():
	mesh = color(mesh, normal_diff, vertex_diff, ignored_materials, prebuilt_materials)

func randomize_face_data():
	if mesh == null:
		return
	...
	for surface in mesh.get_surface_count():
		...
		for i in colors.size():
			var old_c: Color = colors[i]
			
			# if this color hasn’t been mapped yet, generate one
			if not color_map.has(old_c):
				color_map[old_c] = Color(randf(), randf(), randf(), old_c.a) # preserve alpha
			
			# replace in place
			colors[i] = color_map[old_c]
		
		# arrays[Mesh.ARRAY_COLOR] = colors
		
		# rebuild surface with updated colors
		var material = mesh.surface_get_material(surface)
		mesh.surface_remove(surface)
		mesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
		mesh.surface_set_material(mesh.get_surface_count() - 1, material)

func randomize_face_data_old():
	var new_mesh = ArrayMesh.new()
	...
	for surface_id in mesh.get_surface_count():
		var colors = {}
		var mesh_data = mesh_data_surfaces[surface_id]
		var surface_tool = SurfaceTool.new()
		surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)
		
		for face in mesh_data.get_face_count():
			var v1 = mesh_data.get_vertex(mesh_data.get_face_vertex(face, 0))
			var v2 = mesh_data.get_vertex(mesh_data.get_face_vertex(face, 1))
			var v3 = mesh_data.get_vertex(mesh_data.get_face_vertex(face, 2))
			var v1_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face, 0))
			var v2_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face, 1))
			var v3_n = mesh_data.get_vertex_normal(mesh_data.get_face_vertex(face, 2))
			var v_color = mesh_data.get_vertex_color(mesh_data.get_face_vertex(face, 0))

			if colors.has(v_color):
				surface_tool.set_color(colors[v_color])
			else:
				colors[v_color] = Color(randf(), randf(), randf())
				surface_tool.set_color(colors[v_color])
			...
			surface_tool.add_vertex(v3)
		surface_tool.commit(new_mesh)
		new_mesh.surface_set_material(new_mesh.get_surface_count()-1, get_active_material(surface_id))
	mesh = new_mesh

func clear_face_data():
	var new_mesh = ArrayMesh.new()
